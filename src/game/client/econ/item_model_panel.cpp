	...
	if ( !m_bNameOnly )
	{
		...
		if ( pDescription )
		{
			...
			for ( unsigned int i = 0; i < pDescription->GetLineCount(); i++ )
			{
				...
				{
					if ( line.eColor == ATTRIB_COL_UNUSUAL || line.eColor == ATTRIB_COL_STRANGE || line.eColor == ATTRIB_COL_COMMUNITY || line.eColor == ATTRIB_COL_SOURCEMOD || line.eColor == ATTRIB_COL_CUSTOMITEM)
					{
